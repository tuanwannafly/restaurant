...
/**
 * Màn hình phục vụ bàn dành cho role WAITER — Phase 4B.
 * <p>
 * Gồm 3 tab:
 * <ol>
 *   <li><b>Phục vụ bàn</b> – danh sách lượt bàn có món READY, cho phép
 *       chuyển sang DELIVERING / DELIVERED.</li>
 *   <li><b>Dọn bàn</b>     – placeholder, Phase tiếp theo.</li>
...
public class WaiterServicePanel extends JPanel {
    ...
    public void loadData() {
        new SwingWorker<Map<String, List<KitchenDAO.KitchenTicket>>, Void>() {
            @Override
            protected Map<String, List<KitchenDAO.KitchenTicket>> doInBackground() {
                long rid = AppSession.getInstance().getRestaurantId();
                return kitchenDAO.getReadyByTable(rid);
            }

            @Override
            protected void done() {
                try {
                    rebuildDeliveryCards(get());
    ...
    private void rebuildDeliveryCards(
        ...
        // Toast khi phát hiện lượt bàn mới
    ...
    private JPanel buildDeliveryCard(List<KitchenDAO.KitchenTicket> tickets) {
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected void done() {
                    try { get(); } catch (Exception ex) {
                        System.err.println("[WaiterServicePanel] btnDeliv: "
                                + ex.getMessage());
                    }
                    loadData();
                }
        ...
        btnDone.addActionListener(e -> {
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected void done() {
                    try { get(); } catch (Exception ex) {
                        System.err.println("[WaiterServicePanel] btnDone: "
                                + ex.getMessage());
                    }
                    ToastNotification.show(
    ...
    /** FlowLayout tự động xuống dòng khi hết chiều ngang. */
